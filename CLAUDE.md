...
**Product image framing:** source photos must be **1:1 square, 2048×2048px, JPEG or PNG, sRGB, on a seamless white background, with the product centred and spanning ~85% of the frame width**. The uploader accepts only `image/jpeg` and `image/png` (`ProductMediaUploader.tsx`), so iPhone HEIC and WebP are rejected outright. `ProductCard` (collections page + similar wallets) renders that in an `aspect-square` box with **`object-contain`** — contain, not cover, so an off-spec upload letterboxes instead of being silently sliced; with a compliant square source the two are identical. `ProductCardSkeleton` mirrors the ratio and must be changed with it.
